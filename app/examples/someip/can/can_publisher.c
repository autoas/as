...
/* ================================ [ FUNCTIONS ] ============================================== */
boolean
Sd_ServerServiceCANBus_CRMC(PduIdType pduID, uint8_t type, uint16_t serviceID, uint16_t instanceID,
...
void CANBus_OnCanFrameSubscribed(boolean isSubscribe, TcpIp_SockAddrType *RemoteAddr) {
}

void SomeIp_CANBus_CanFrame_Notify(void) {
  Std_ReturnType ercd = E_NOT_OK;
  static CanFrame_Type canFrame = {0, 0, 8, {0}};
  int32_t length;

  for (length = 0; length < 8; length++) {
    canFrame.data[length] = (canFrame.canid + length) & 0xFF;
  }

  ercd = CANBus_SendCanFrame(&canFrame);
  if (E_OK == ercd) {
    ASLOG(CANBUS, ("publish: canid = 0x%x, dlc = %d,"
                   " data = [%02X, %02X, %02X, %02X,%02X, %02X, %02X, %02X]\n",
                   canFrame.canid, canFrame.dlc, canFrame.data[0], canFrame.data[1],
