...
class dcm:
    ...
    def __init__(self, **kwargs):
        ...
        if self.verbose:
            self.maxlen = kwargs.get("maxlen", 32)
            ...
            if isinstance(log, logging.Logger):
                ...
                if log == sys.stdout:
                    ...
                    handler = RotatingFileHandler(filename=log, maxBytes=2 * 1024 * 1024, backupCount=10)
    ...
    def __show_negative_response__(self, res):
        if (res[0] == 0x7F) and (len(res) == 3):
            ...
            self.logger.error("unknown response: ", ",".join(["%02X" % (d) for d in res]))

    def __show_request__(self, req):
        if False == self.verbose:
            return
        ss = ">> dcm request %s = [" % (self.__get_service_name__(req[0]))
        ss += ",".join(["%02X" % (d) for d in req[:self.maxlen]])
        if len(req) > self.maxlen:
            ss += ",..."
        ss += "]"
        ss += " len = %d" % (len(req))
        self.logger.info(ss)
    ...
    def __show_response__(self, res):
        if False == self.verbose:
            return
        ss = ">> dcm response = ["
        ss += ",".join(["%02X" % (d) for d in res[:self.maxlen]])
    ...
    def transmit(self, req, waitResponse=True, timeoutMs=5000):
        self.last_error = None
        self.last_reponse = None
        response = None
        self.__show_request__(req)
        ercd = self.tp.transmit(bytes(req), timeoutMs)
        if (len(req) >= 2) and (req[0] in self.__sbr__) and ((req[1] & 0x80) != 0):
            # suppress positive response
            return True, [req[0] | 0x40]
        while (ercd == True) and (waitResponse == True):
            res = self.tp.receive(timeoutMs)
            if res != None:
