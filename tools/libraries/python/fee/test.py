...
def monitor_admin_info():
    admin_info = PyFee.admin_info()
    curWrokingBank = admin_info["curWrokingBank"]
    if curWrokingBank not in lastAdminInfo:
        lastAdminInfo[curWrokingBank] = admin_info
        print("  %s" % (admin_info))
    last_info = lastAdminInfo[curWrokingBank]
    if last_info["erasedNumber"] != admin_info["erasedNumber"]:
        print("  %s" % (admin_info))
    lastAdminInfo[curWrokingBank] = admin_info
...
def resume(**kwargs):
    ...
    for name in blocks.keys():
        ...
        print("read %s:%s" % (name, format(["%02X" % (d) for d in data])))
    PyFee.power_off()
...
def normal(**kwargs):
    ...
    for n in range(nLoops):
        ...
        for i in range(NUM_BLOCKS):
            name = str(i)
            data = bytes([random.randint(0, 255) for i in range(blocks[name])])
            lastDatas[name] = data
        ...
        for i in range(NUM_BLOCKS):
            ...
            if data != lastDatas[name]:
                with open("Fls.img", "wb") as f:
                    f.write(PyFee.img())
                raise Exception("%s loops, %s: not equal, %s %s" % (n, name, data, lastDatas[name]))
        PyFee.power_off()
    ...
    for n in range(nLoops):
        ...
        for i in range(NUM_BLOCKS):
            ...
            if not any(data == d for d in lastDatas[name]):
                ...
                str2 = format([["%02X" % (d) for d in D] for D in lastDatas[name]])
                strh = format([["%02X" % (d) for d in D] for D in history[name]])
                raise Exception("%s loops, %s: not equal, %s\n\t%s\n\thistory: %s" % (n, name, str1, str2, strh))
            lastDatas[name] = [data]
        PyFee.power_off()
