...
#define DECL_STDXL_FUNC(apiname, apitype, args) typedef XLstatus(_XL_EXPORT_API *apitype) args
...
// All DECL_STDXL_FUNC have return type XLstatus
#define DECL_STDXL_FUNC(apiname, apitype, args) XLstatus _XL_EXPORT_DECL apiname args
...
#define XL_TRANSCEIVER_TYPE_CAN_VIRTUAL                                                            \
  0x0016 //!< Virtual CAN Trasceiver for Virtual CAN Bus Driver
#define XL_TRANSCEIVER_TYPE_LIN_6258_OPTO                                                          \
  0x0017 //!< Vector LINcab 6258opto with transceiver Infineon TLE6258
#define XL_TRANSCEIVER_TYPE_LIN_6259_OPTO                                                          \
  0x0019 //!< Vector LINcab 6259opto with transceiver Infineon TLE6259
#define XL_TRANSCEIVER_TYPE_DAIO_8444_OPTO                                                         \
  0x001D //!< Vector IOcab 8444  (8 dig.Inp.; 4 dig.Outp.; 4 ana.Inp.; 4 ana.Outp.)
#define XL_TRANSCEIVER_TYPE_CAN_1041A_OPTO 0x0021 //!< 1041A with optical isolation
#define XL_TRANSCEIVER_TYPE_LIN_6259_MAG                                                           \
  0x0023 //!< LIN transceiver 6259, with transceiver Infineon TLE6259, magnetically isolated, stress
         //!< functionality
#define XL_TRANSCEIVER_TYPE_LIN_7259_MAG                                                           \
  0x0025 //!< LIN transceiver 7259, with transceiver Infineon TLE7259, magnetically isolated, stress
         //!< functionality
#define XL_TRANSCEIVER_TYPE_LIN_7269_MAG                                                           \
  0x0027 //!< LIN transceiver 7269, with transceiver Infineon TLE7269, magnetically isolated, stress
         //!< functionality
#define XL_TRANSCEIVER_TYPE_CAN_1054_MAG                                                           \
  0x0033 //!< TJA1054, magnetically isolated, with selectable termination resistor (via 4th IO line)
...
#define XL_TRANSCEIVER_TYPE_TWIN_CAN_1041A_MAG                                                     \
  0x0080 //!< TWINcab with two TJA1041, magnetically isolated
...
#define XL_TRANSCEIVER_TYPE_TWIN_CAN_1041AV2_MAG                                                   \
  0x0082 //!< TWINcab with two TJA1041, magnetically isolated
#define XL_TRANSCEIVER_TYPE_TWIN_CAN_1054_1041A_MAG                                                \
  0x0083 //!< TWINcab with TJA1054A and TJA1041A with magnetic isolation
...
#define XL_TRANSCEIVER_TYPE_PB_LIN_6258_OPTO                                                       \
  0x0129 //!< LIN piggy back with transceiver Infineon TLE6258
#define XL_TRANSCEIVER_TYPE_PB_LIN_6259_OPTO                                                       \
  0x012B //!< LIN piggy back with transceiver Infineon TLE6259
#define XL_TRANSCEIVER_TYPE_PB_LIN_6259_MAG                                                        \
  0x012D //!< LIN piggy back with transceiver Infineon TLE6259, magnetically isolated, stress
         //!< functionality
#define XL_TRANSCEIVER_TYPE_PB_CAN_1041A_OPTO 0x012F //!< CAN transceiver 1041A
#define XL_TRANSCEIVER_TYPE_PB_LIN_7259_MAG                                                        \
  0x0131 //!< LIN piggy back with transceiver Infineon TLE7259, magnetically isolated, stress
         //!< functionality
#define XL_TRANSCEIVER_TYPE_PB_LIN_7269_MAG                                                        \
  0x0133 //!< LIN piggy back with transceiver Infineon TLE7269, magnetically isolated, stress
         //!< functionality
#define XL_TRANSCEIVER_TYPE_PB_CAN_251_MAG                                                         \
  0x0135 //!< 82C250/251 or compatible, magnetically isolated
...
#define XL_TRANSCEIVER_TYPE_CAN_1051_CAP_FIX                                                       \
  0x013C                                         //!< TJA1051 - fixed transceiver on e.g. 16xx/8970
...
#define XL_TRANSCEIVER_TYPE_PB_FRC_1082_CAP                                                        \
  0x0205 //!< TJA 1082 capacitive isolated piggy with CANpiggy form factor
#define XL_TRANSCEIVER_TYPE_FR_1082_CAP_FIX                                                        \
  0x0206 //!< TJA 1082 capacitive isolated piggy fixed transceiver - e.g. 7610

#define XL_TRANSCEIVER_TYPE_MOST150_ONBOARD 0x0220 //!< Onboard MOST150 transceiver of VN2640

// Ethernet Phys
#define XL_TRANSCEIVER_TYPE_ETH_BCM54810_FIX                                                       \
  0x0230 //!< Onboard Broadcom Ethernet PHY on VN5610 and VX0312
...
#define XL_TRANSCEIVER_TYPE_DAIO_AL_ONLY                                                           \
  0x028f //!< virtual piggy type for activation line only (e.g. VN8810ini)
#define XL_TRANSCEIVER_TYPE_DAIO_1021_FIX_WITH_AL                                                  \
  0x0290 //!< On board IO with Activation Line (e.g. VN5640)
#define XL_TRANSCEIVER_TYPE_DAIO_AL_WU                                                             \
  0x0291 //!< virtual piggy type for activation line and WakeUp Line only (e.g. VN5610A)
...
#define XL_TRANSCEIVER_LINEMODE_TT_CAN_H                                                           \
  ((unsigned int)0x000b) //!< truck & trailer: operating mode single wire using CAN high
#define XL_TRANSCEIVER_LINEMODE_TT_CAN_L                                                           \
  ((unsigned int)0x000c) //!< truck & trailer: operating mode single wire using CAN low
...
#define XL_ERR_INVALID_FDFLAG_MODE20                                                               \
  0x0203                      // flag set that must not be set when configured for CAN20 (e.g. EDL)
...
#define XL_ERR_ETH_SET_CONFIG_DELAYED                                                              \
  0x1105 // Requested config was stored but could not be immediately activated
...
#define XL_ERR_ETH_FILTER_UNAVAILABLE                                                              \
  0x1109 // Filtering is not available (not supported by hardware, in use by other application, ...)
#define XL_ERR_ETH_FILTER_NO_INIT_ACCESS                                                           \
  0x110A // Missing init access for one or more channels referenced by filter
#define XL_ERR_ETH_FILTER_TOO_COMPLEX                                                              \
  0x110B // Specified filter setting is too complex to be fully supported
...
#define XL_ETH_EVENT_TAG_FRAMERX_ERROR                                                             \
  ((unsigned short)0x0501) // Event data type T_XL_ETH_DATAFRAME_RX_ERROR
#define XL_ETH_EVENT_TAG_FRAMETX_ERROR                                                             \
  ((unsigned short)0x0506) // Event data type T_XL_ETH_DATAFRAME_TX_ERROR
#define XL_ETH_EVENT_TAG_FRAMETX_ERROR_SWITCH                                                      \
  ((unsigned short)0x0507) // Event data type T_XL_ETH_DATAFRAME_TX_ERR_SW
#define XL_ETH_EVENT_TAG_FRAMETX_ACK                                                               \
  ((unsigned short)0x0510) // Event data type T_XL_ETH_DATAFRAME_TXACK
#define XL_ETH_EVENT_TAG_FRAMETX_ACK_SWITCH                                                        \
  ((unsigned short)0x0511) // Event data type T_XL_ETH_DATAFRAME_TXACK_SW
#define XL_ETH_EVENT_TAG_FRAMETX_ACK_OTHER_APP                                                     \
  ((unsigned short)0x0513) // Event data type T_XL_ETH_DATAFRAME_TXACK_OTHERAPP
#define XL_ETH_EVENT_TAG_FRAMETX_ERROR_OTHER_APP                                                   \
  ((unsigned short)0x0514) // Event data type T_XL_ETH_DATAFRAME_TX_ERR_OTHERAPP
#define XL_ETH_EVENT_TAG_CHANNEL_STATUS                                                            \
  ((unsigned short)0x0520) // Event data type T_XL_ETH_CHANNEL_STATUS
#define XL_ETH_EVENT_TAG_CONFIGRESULT                                                              \
  ((unsigned short)0x0530) // Event data type T_XL_ETH_CONFIG_RESULT
#define XL_ETH_EVENT_TAG_LOSTEVENT                                                                 \
  ((unsigned short)0x05fe) // Indication that one or more intended events could not be generated.
                           // Event data type T_XL_ETH_LOSTEVENT
...
#define XL_LIN_CALC_CHECKSUM                                                                       \
  (unsigned short)0x100 //!< flag for automatic 'classic' checksum calculation
#define XL_LIN_CALC_CHECKSUM_ENHANCED                                                              \
  (unsigned short)0x200 //!< flag for automatic 'enhanced' checksum calculation
...
#define XL_LIN_SET_WAKEUPID                                                                        \
  (unsigned int)0x03 //!< set hardware into sleep mode and send a request at wake-up
...
#define XL_LIN_CHECKSUM_UNDEFINED                                                                  \
  (unsigned char)0xff //!< Set the checksum calculation to undefined.

// defines for the sleep mode event: XL_LIN_SLEEP
#define XL_LIN_STAYALIVE (unsigned char)0x00 //!< flag if nothing changes
#define XL_LIN_SET_SLEEPMODE                                                                       \
  (unsigned char)0x01 //!< flag if the hardware is set into the sleep mode
#define XL_LIN_COMESFROM_SLEEPMODE                                                                 \
  (unsigned char)0x02 //!< flag if the hardware comes from the sleep mode
...
#define XL_CAN_MSG_FLAG_OVERRUN                                                                    \
  0x02                              //!< Overrun in Driver or CAN Controller,
...
// This structure is returned as an array from the xlGetLicenseInfo. It contains all available
// licenses on the queried channels. The position inside the array is defined by the license itself,
// e.g. the license for the Advanced-Flexray-Library is always at the same array index.
...
typedef struct s_xl_channel_config {
  ...
  unsigned short
    channelBusActiveCapabilities; //!< like channelBusCapabilities (but without core dependencies)
...
#define XL_DAIO_ANALOG_RANGE_32V                                                                   \
  0x00000008 // analog port is in range 0..32,768V, otherwise 0..8,192V
...
#define XL_DAIO_TRIGGER_MODE_ANALOG                                                                \
...
#define XL_REMOTE_DEVICE_TEMP_CONFIGURED                                                           \
  0x00000020 // the device is temporary configured, it has not entry in registry
...
#define XL_CHANNEL_FLAG_NO_HWSYNC_SUPPORT                                                          \
  0x00000400 // Device is not capable of hardware-based time synchronization via Sync-line
...
#define XL_MOST_QUEUE_OVERFLOW                                                                     \
  0x8000 // Overflow of rx-queue in firmware when trying to add a rx-event
...
#define XL_MOST_QUEUE_OVERFLOW_ASYNC                                                               \
  0x0800 // Overflow of async rx-queue in firmware when trying to add a packet
#define XL_MOST_QUEUE_OVERFLOW_CTRL                                                                \
  0x0400 // Overflow of rx-queue in firmware when trying to add a message
#define XL_MOST_NOT_SUPPORTED 0x0200
#define XL_MOST_QUEUE_OVERFLOW_DRV                                                                 \
  0x0100 // Overflow occured when trying to add an event to application rx-queue
...
#define XL_MOST_STREAM_ADD_FRAME_HEADER                                                            \
  1 // only for RX: additionally the orig. TS + status information are reported
...
typedef struct s_xl_fr_cluster_configuration {
  ...
  unsigned int
    framePresetData; //!< 16-bit value with data for pre-initializing the Flexray payload data words
...
#define XL_FR_MODE_COLD_NORMAL                                                                     \
  0x04 //!< setup the VN3000 (Fujitsu) normal operation mode. (default mode)
...
#define XL_FR_MODE_COLDSTART_LEADING                                                               \
  0x02 //!< Coldstart path initiating the schedule synchronization
#define XL_FR_MODE_COLDSTART_FOLLOWING 0x03 //!< Coldstart path joining other coldstart nodes
#define XL_FR_MODE_WAKEUP_AND_COLDSTART_LEADING                                                    \
  0x04 //!< Send Wakeup and Coldstart path initiating the schedule synchronization
#define XL_FR_MODE_WAKEUP_AND_COLDSTART_FOLLOWING                                                  \
...
typedef struct s_xl_fr_acceptance_filter {
  unsigned int
    filterStatus; //!< defines if the specified frame should be blocked or pass the filter
...
#define XL_FR_FRAMEFLAG_PAYLOAD_PREAMBLE                                                           \
  ((unsigned short)0x0008) //!< indicates a present payload preamble bit
...
#define XL_FR_FRAMEFLAG_TXACK_SS                                                                   \
  XL_FR_FRAMEFLAG_REQ_TXACK //!< indicates TxAck of SingleShot; used for TxAck events only
#define XL_FR_FRAMEFLAG_RX_UNEXPECTED                                                              \
  XL_FR_FRAMEFLAG_REQ_TXACK //!< indicates unexpected Rx frame; used for Rx events only

#define XL_FR_FRAMEFLAG_NEW_DATA_TX                                                                \
  ((unsigned short)0x0040) //!< flag used with TxAcks to indicate first TxAck after data update
#define XL_FR_FRAMEFLAG_DATA_UPDATE_LOST                                                           \
...
#define XL_FR_FRAMEFLAG_FRAME_TRANSMITTED                                                          \
  ((unsigned short)0x8000) //!< Only used with TxAcks: Frame has been transmitted. If not set after
                           //!< transmission, an error has occurred.
...
#define XL_FR_STATUS_DEFAULT_CONFIG                                                                \
  0x00                          //!< indicates the actual state of the POC in operation control
...
#define XL_FR_STATUS_WAKEUP_STANDBY                                                                \
  0x10 //!< indicates the actual state of the POC in the wakeup path
#define XL_FR_STATUS_WAKEUP_LISTEN 0x11 //!< ...
#define XL_FR_STATUS_WAKEUP_SEND 0x12   //!< ...
#define XL_FR_STATUS_WAKEUP_DETECT 0x13 //!< ...

#define XL_FR_STATUS_STARTUP_PREPARE                                                               \
  0x20 //!< indicates the actual state of the POC in the startup path
...
#define XL_FR_ERROR_POC_PASSIVE                                                                    \
  0x01 //!< Indicates the actual error mode of the POC: passive (yellow)
#define XL_FR_ERROR_POC_COMM_HALT                                                                  \
  0x02 //!< Indicates the actual error mode of the POC: comm-halt (red)
...
#define XL_FR_WAKEUP_UNDEFINED                                                                     \
  0x00 //!< No wakeup attempt since CONFIG state was left. (e.g. when a wakeup pattern A|B is
       //!< received)
#define XL_FR_WAKEUP_RECEIVED_HEADER 0x01 //!< Frame header without coding violation received.
#define XL_FR_WAKEUP_RECEIVED_WUP                                                                  \
  0x02 //!< Wakeup pattern on the configured wakeup channel received.
#define XL_FR_WAKEUP_COLLISION_HEADER                                                              \
...
struct s_xl_fr_event {
  ...
  unsigned short
    flagsChip; // 2 - frChannel e.g. XL_FR_CHANNEL_A (lower 8 bit), queue overflow (upper 8bit)
  unsigned short reserved; // 2
...
typedef struct xl_daio_set_port {
  unsigned int
    portType; //!< Only one signal group is allowed. One of the defines XL_DAIO_PORT_TYPE_MASK_*
  unsigned int portMask;        //!< Mask of affected ports.
  unsigned int portFunction[8]; //!< Special function of port. One of the defines
                                //!< XL_DAIO_PORT_DIGITAL_* or XL_DAIO_PORT_ANALOG_*
...
#define XL_ETH_RX_FIFO_QUEUE_SIZE_MAX                                                              \
  (unsigned int)(8 * 1024 * 1024) //!< Maximum size of ethernet receive queue: 8 MByte
#define XL_ETH_RX_FIFO_QUEUE_SIZE_MIN                                                              \
  (unsigned int)(64 * 1024) //!< Minimum size of ethernet receive queue: 64 KByte

#define XL_ETH_PAYLOAD_SIZE_MAX                                                                    \
  (unsigned int)1500 // maximum payload length for sending an ethernet packet
#define XL_ETH_PAYLOAD_SIZE_MIN                                                                    \
  (unsigned int)46 // minimum payload length for sending an ethernet packet (42 octets with VLAN tag
                   // present)
#define XL_ETH_RAW_FRAME_SIZE_MAX                                                                  \
  (unsigned int)1600 // maximum buffer size for storing a "raw" Ethernet frame (including VLAN tags,
                     // if present)
...
#define XL_ETH_BYPASS_QUEUE_OVERFLOW                                                               \
  (unsigned short)0x8000 // MAC bypass queue full condition occurred, one or more packets dropped
...
#define XL_ETH_MODE_SPEED_AUTO_100_1000                                                            \
  5 /* Set connection speed automatically to either 100 or 1000 Mbps */
#define XL_ETH_MODE_SPEED_FIXED_100                                                                \
  8 /* Set connection speed to 100 Mbps. Auto-negotiation disabled. */

// duplex
#define XL_ETH_MODE_DUPLEX_DONT_CARE                                                               \
  0 /* Used for BroadR-Reach since only full duplex mode possible. */
#define XL_ETH_MODE_DUPLEX_AUTO                                                                    \
  1 /* Duplex mode set via auto-negotiation. Requires connection speed set to an "auto" value.     \
       Only for IEEE 802.3*/
...
#define XL_ETH_MODE_CLOCK_AUTO                                                                     \
  1 /* Clock mode set automatically via auto-negotiation. Only for 1000Base-T if speed mode is one \
       of the "auto" modes */
...
#define XL_ETH_STATUS_LINK_UNKNOWN                                                                 \
  0 /* The link state could not be determined (e.g. lost connection to board) */
#define XL_ETH_STATUS_LINK_DOWN                                                                    \
  1 /* Link is down (no cable attached, no configuration set, configuration does not match) */
...
#define XL_ETH_STATUS_SPEED_UNKNOWN                                                                \
  0 /* Connection speed could not be determined (e.g. during auto-negotiation or if link down) */
...
#define XL_ETH_STATUS_DUPLEX_UNKNOWN                                                               \
  0 /* Duplex mode could not be determined (e.g. during auto-negotiation or if link down) */
#define XL_ETH_STATUS_DUPLEX_FULL 2 /* Full duplex mode */

// T_XL_ETH_CHANNEL_STATUS.mdiType
#define XL_ETH_STATUS_MDI_UNKNOWN                                                                  \
  0 /* MDI mode could not be determined  (e.g. during auto-negotiation or if link down) */
...
#define XL_ETH_STATUS_CLOCK_DONT_CARE                                                              \
  0                                  /* Clock mode not relevant. Only for IEEE 802.3 100/10 MBit */
...
#define XL_ETH_STATUS_BR_PAIR_DONT_CARE                                                            \
  0 /* No BR pair available. Only for IEEE 802.3 1000/100/10 MBit */
...
#define XL_ETH_RX_ERROR_INVALID_LENGTH                                                             \
  ((unsigned int)0x00000001) /* Invalid length error. Set when the receive frame has an invalid    \
                                length as defined by IEEE802.3 */
#define XL_ETH_RX_ERROR_INVALID_CRC                                                                \
  ((unsigned int)0x00000002) /* CRC error. Set when frame is received with CRC-32 error but valid  \
                                length */
#define XL_ETH_RX_ERROR_PHY_ERROR                                                                  \
  ((unsigned int)0x00000004) /* Corrupted receive frame caused by a PHY error */
...
#define XL_ETH_DATAFRAME_FLAGS_USE_SOURCE_MAC                                                      \
  (unsigned int)0x00000001 /* Use the given source MAC address (not set by hardware) */
...
typedef struct s_xl_eth_dataframe_tx {
  ...
  unsigned int flags;           /* Flags to specify whether to use the given source MAC and FCS (see
                                   XL_ETH_DATAFRAME_FLAGS_) */
  unsigned short dataLen;       /* Overall data length of <frameData> */
...
typedef struct s_xl_eth_dataframe_tx_event {
  unsigned int frameIdentifier; /* FPGA internal identifier unique to every frame sent */
  unsigned int flags; /* Flags signalizing whether the given source MAC and FCS have been used by
                         the send request (see XL_ETH_DATAFRAME_FLAGS_) */
  unsigned short dataLen;     /* Overall data length of <frameData> */
  unsigned short reserved;    /* currently reserved field - not used, ignore */
  unsigned int frameDuration; /* transmit duration of the Ethernet frame, in nanoseconds */
...
typedef struct s_xl_eth_channel_status {
  ...
  unsigned int duplex;  /* (XL_ETH_STATUS_DUPLEX_*)    Ethernet duplex mode. 1000Base-T always uses
                           full duplex. */
  unsigned int mdiType; /* (XL_ETH_STATUS_MDI_*)       Currently active MDI-mode */
  unsigned int activeConnector; /* (XL_ETH_STATUS_CONNECTOR_*) Connector (plug) to use (BroadR-REACH
                                   or RJ-45). */
  unsigned int activePhy;       /* (XL_ETH_STATUS_PHY_*)       Currently active physical layer */
  unsigned int clockMode;       /* (XL_ETH_STATUS_CLOCK_*)     When in 1000Base-T or BroadR-mode,
                                   currently active mode */
  unsigned int
    brPairs; /* (XL_ETH_STATUS_BR_PAIR_*)   When in BroadR-mode, number of used cable pairs */
...
typedef struct s_xl_eth_event {
  unsigned int
    size; // 4 - overall size of the complete event, depending on event type and piggybacked data
  ...
  union s_xl_eth_tag_data {
    ...
    T_XL_ETH_DATAFRAME_RX
      frameRxOk; //(tag==XL_ETH_EVENT_TAG_FRAMERX)              Frame received from network
    T_XL_ETH_DATAFRAME_RX_ERROR frameRxError; //(tag==XL_ETH_EVENT_TAG_FRAMERX_ERROR) Erroneous
                                              //frame received from network
    T_XL_ETH_DATAFRAME_TXACK
      frameTxAck; //(tag==XL_ETH_EVENT_TAG_FRAMETX_ACK)          ACK for frame sent by application
    T_XL_ETH_DATAFRAME_TXACK_SW
      frameTxAckSw; //(tag==XL_ETH_EVENT_TAG_FRAMETX_ACK_SWITCH)   ACK for frame sent by switch
    T_XL_ETH_DATAFRAME_TXACK_OTHERAPP
      frameTxAckOtherApp; //(tag==XL_ETH_EVENT_TAG_FRAMETX_ERROR_OTHER_APP) ACK for frame sent by
                          //another application
    T_XL_ETH_DATAFRAME_TX_ERROR
      frameTxError; //(tag==XL_ETH_EVENT_TAG_FRAMETX_ERROR)        NACK for frame sent by
                    //application (frame could not be transmitted)
    T_XL_ETH_DATAFRAME_TX_ERR_SW
      frameTxErrorSw; //(tag==XL_ETH_EVENT_TAG_FRAMETX_ERROR_SWITCH) NACK for frame sent by switch.
                      //May indicate internal processing failure (e.g. queue full condition)
    T_XL_ETH_DATAFRAME_TX_ERR_OTHERAPP
      frameTxErrorOtherApp; //(tag==XL_ETH_EVENT_TAG_FRAMETX_ERROR_OTHER_APP) NACK for frame sent by
                            //another application
    T_XL_ETH_CONFIG_RESULT configResult;
    T_XL_ETH_CHANNEL_STATUS channelStatus;
    XL_SYNC_PULSE_EV syncPulse;
    T_XL_ETH_LOSTEVENT lostEvent; //(tag==XL_ETH_EVENT_TAG_LOSTEVENT)           Indication that one
...
typedef struct {
  ...
  unsigned int duplex;    /* (XL_ETH_MODE_DUPLEX_*)      Duplex mode setting. Not relevant for
                             BroadR-REACH mode, set to "nochange" or "auto". */
  ...
  unsigned int clockMode; /* (XL_ETH_MODE_CLOCK_*)       Master or slave clock mode setting
                             (1000Base-T/BroadR-REACH mode only). */
  unsigned int mdiMode;   /* (XL_ETH_MODE_MDI_*)         Currently active MDI-mode */
  unsigned int brPairs;   /* (XL_ETH_MODE_BR_PAIR_*)     Number of cable pairs to use (BroadR-REACH
                             mode only). */
} T_XL_ETH_CONFIG;
...
#define XL_MOST150_ASYNC_PAYLOAD_MAX_SIZE                                                          \
  (unsigned short)1524 // maximum valid length (s. INIC User Manual)
#define XL_MOST150_ETHERNET_PAYLOAD_MAX_SIZE                                                       \
  (unsigned short)1506 // maximum valid length (s. INIC User Manual)
#define XL_MOST150_ASYNC_SEND_PAYLOAD_MAX_SIZE                                                     \
  (unsigned short)1600 // maximum length for sending a MDP
#define XL_MOST150_ETHERNET_SEND_PAYLOAD_MAX_SIZE                                                  \
  (unsigned short)1600 // maximum length for sending a MEP
...
#define XL_MOST150_QUEUE_OVERFLOW                                                                  \
  (unsigned short)0x0100 //!< queue overflow occured (some events are lost)
...
#define XL_MOST150_ASYNC_INVALID_RX_LENGTH                                                         \
  (unsigned int)0x00008000 // flag indicating a received MDP with length >
                           // XL_MOST150_ASYNC_PAYLOAD_MAX_SIZE

// XL_MOST150_ETHERNET_RX_EV.length
#define XL_MOST150_ETHERNET_INVALID_RX_LENGTH                                                      \
  (unsigned int)0x80000000 // flag indicating a received MEP with length >
                           // XL_MOST150_ETHERNET_PAYLOAD_MAX_SIZE
...
// special CL for xlMost150CtrlSyncAudio to de-allocate all CLs for device
...
typedef struct s_xl_most150_ctrl_tx_msg {
  ...
  unsigned int ctrlSendAttempts; // 1..16 attempts, set an invalid value to use the default value
                                 // set by xlMost150SetCtrlRetryParameters
...
typedef struct s_xl_most150_async_tx_msg {
  ...
  unsigned int asyncSendAttempts; // 1..16 attempts,set an invalid value to use the default value
                                  // set by xlMost150SetAsyncRetryParameters
...
typedef struct s_xl_most150_ethernet_tx_msg {
  unsigned int priority;        // Prio: Currently fixed to 0x00 for MDP /MEP
  unsigned int ethSendAttempts; // 1..16 attempts, set an invalid value to use the default value set
                                // by xlMost150SetAsyncRetryParameters
  unsigned char sourceAddress[6];
  unsigned char targetAddress[6];
  unsigned int length; // max. 1600 bytes
...
// XLstatus xlMost150CtrlConfigureBusload(DEFPARAMS, XLmost150CtrlBusloadConfig
// *pCtrlBusloadConfig);
typedef struct s_xl_most150_ctrl_busload_config {
  unsigned int transmissionRate;
  unsigned int counterType;
  unsigned int counterPosition; // counter can be only be set in the payload -> position 0 means
                                // first payload byte!
  XLmost150CtrlTxMsg busloadCtrlMsg;
} XLmost150CtrlBusloadConfig;

// XLstatus xlMost150AsyncConfigureBusload(DEFPARAMS, XLmost150AsyncBusloadConfig
// *pAsyncBusloadConfig);
...
#define XL_CAN_TXMSG_FLAG_HIGHPRIO                                                                 \
  0x0080 // high priority message - clears all send buffers - then transmits
...
#define XL_CAN_RXMSG_FLAG_EF                                                                       \
  0x0200 // error frame (only posssible in XL_CAN_EV_TX_REQUEST/XL_CAN_EV_TX_REMOVED)
#define XL_CAN_RXMSG_FLAG_ARB_LOST                                                                 \
  0x0400 // Arbitration Lost
...
#define XL_A429_MSG_PARITY_DISABLED                                                                \
  1 // tx: get parity config from transmit data - rx: check disabled
...
DECL_STDXL_FUNC(xlGetApplConfig, XLGETAPPLCONFIG,
                (char *appName,            //<! Name of Application
                 ...
                 unsigned int busType //<! Bus type of configuration, should be BUS_TYPE_NONE when
                                      //no bus type is set
                 ));
...
#else
int _XL_EXPORT_DECL xlGetChannelIndex(int hwType,   // [-1,HWTYPE_CANCARDX,HWTYPE_VIRTUAL,...]
                                      int hwIndex,  // [-1,0,1]
                                      int hwChannel // [-1,0,1]
);

XLaccess _XL_EXPORT_DECL xlGetChannelMask(int hwType,   // [-1,HWTYPE_CANCARDX,HWTYPE_VIRTUAL,...]
                                          int hwIndex,  // [-1,0,1]
                                          int hwChannel // [-1,0,1]
);
#endif
...
DECL_STDXL_FUNC(xlOpenPort, XLOPENPORT,
                (XLportHandle * pPortHandle, char *userName, XLaccess accessMask,
                 XLaccess *pPermissionMask, unsigned int rxQueueSize,
...
DECL_STDXL_FUNC(xlSetTimerRate, XLSETTIMERRATE, (XLportHandle portHandle, unsigned long timerRate));
...
DECL_STDXL_FUNC(xlActivateChannel, XLACTIVATECHANNEL,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int busType,
                 unsigned int flags));
...
DECL_STDXL_FUNC(xlOemContact, XLOEMCONTACT,
                (XLportHandle portHandle, unsigned long Channel, XLuint64 context1,
                 XLuint64 *context2));
 ...
 *  \param lineMode       [IN] Transceiver operation mode (specified by defines
 *'XL_TRANSCEIVER_LINEMODE_*') \param resNet         [IN] Reserved. Should always be set to zero!
 */
...
DECL_STDXL_FUNC(xlCanSetChannelParamsC200, XLCANSETCHANNELPARAMSC200,
                (XLportHandle portHandle, XLaccess accessMask, unsigned char btr0,
                 unsigned char btr1));
...
DECL_STDXL_FUNC(xlCanTransmitEx, XLCANTRANSMITEX,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int msgCnt,
                 unsigned int *pMsgCntSent, XLcanTxEvent *pXlCanTxEvt));
...
DECL_STDXL_FUNC(xlCanSetChannelAcceptance, XLCANSETCHANNELACCEPTANCE,
                (XLportHandle portHandle, XLaccess accessMask, unsigned long code,
                 unsigned long mask, unsigned int idRange));
...
DECL_STDXL_FUNC(xlCanAddAcceptanceRange, XLCANADDACCEPTANCERANGE,
                (XLportHandle portHandle, XLaccess accessMask, unsigned long first_id,
...
DECL_STDXL_FUNC(xlCanTransmit, XLCANTRANSMIT,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int *pEventCount,
                 void *pEvents));
...
*/
DECL_STDXL_FUNC(xlSetGlobalTimeSync, XLSETGLOBALTIMESYNC,
 ...
 *  This function returns the available licenses in an array of XLlicenseInfo structures. This array
 *contains all available licenses on the queried channels. The position inside the array is defined
 *by the license itself, e.g. the license for the Advanced-Flexray-Library is always at the same
 *array index. \param channelMask      [IN] : Channelmask for which to query the available licenses
 *  \param *pLicInfoArray   [OUT]: Array with license overview
 *  \param licInfoArraySize [IN] : Size of array pointed to with 'pLicInfo' (number of array
 *entries) \return XLstatus            General status information is returned. XL_SUCCESS if no
 *error occurred. XL_ERR_NO_RESOURCES if the given array size is too small to copy all available
 *licenses into it. XL_ERROR if general error occurred.
 */
DECL_STDXL_FUNC(xlGetLicenseInfo, XLGETLICENSEINFO,
                (XLaccess channelMask, XLlicenseInfo *pLicInfoArray,
...
DECL_STDXL_FUNC(xlDAIOSetPWMOutput, XLDAIOSETPWMOUTPUT,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int frequency,
                 unsigned int value));
DECL_STDXL_FUNC(xlDAIOSetDigitalOutput, XLDAIOSETDIGITALOUTPUT,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int outputMask,
                 unsigned int valuePattern));
DECL_STDXL_FUNC(xlDAIOSetAnalogOutput, XLDAIOSETANALOGOUTPUT,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int analogLine1,
                 unsigned int analogLine2, unsigned int analogLine3, unsigned int analogLine4));
DECL_STDXL_FUNC(xlDAIORequestMeasurement, XLDAIOREQUESTMEASUREMENT,
                (XLportHandle portHandle, XLaccess accessMask));
DECL_STDXL_FUNC(xlDAIOSetDigitalParameters, XLDAIOSETDIGITALPARAMETERS,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int inputMask,
                 unsigned int outputMask));
DECL_STDXL_FUNC(xlDAIOSetAnalogParameters, XLDAIOSETANALOGPARAMETERS,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int inputMask,
                 unsigned int outputMask, unsigned int highRangeMask));
DECL_STDXL_FUNC(xlDAIOSetAnalogTrigger, XLDAIOSETANALOGTRIGGER,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int triggerMask,
                 unsigned int triggerLevel, unsigned int triggerEventMode));
 ...
 *    XLuserHandle userHandle:             used to match the response of the driver to the requests
 * of the application if an event is received spontaneously, e.g. MPR changed then the userHandle ==
 * 0 Common Return Value: XLstatus:                            common return value of most APIs
 * which indicates whether a command was successfully launched or e.g. whether a command queue
 * overflow happened
 */

/** \brief fetching events from driver queue.
 *  This method is used to fetch events, either bus events or acknowledgments
 *  for commands from the driver queue. Each call delivers only one event (if an event is
 * available). \n It is a synchronous mode and either delivers event data immediately, or indicates
 * an error condition with its return value. \param  XLportHandle portHandle:  [IN] was previously
 * fetched by xlOpenPort API (TODO) \param  XLaccess accessMask:      [IN] determines on which
 * channels an API should work (TODO) \param  pEventBuffer              [IN] This parameter must
 * point to a buffer to which the driver can copy the next event of the receive queue \return
 * XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostReceive, XLFP_MOSTRECEIVE,
 ...
 *  \param sourceMask                 [IN] each bit stands for an event source and can separately be
 * set. Use the definitions of the sourcemask... \return XLstatus                       general
 * status information
 */
DECL_STDXL_FUNC(xlMostSwitchEventSources, XLFP_MOSTSWITCHEVENTSOURCES,
                (DEFPARAMS, unsigned short sourceMask));
 ...
 *  \param  adr                       [IN] address (MAP) of register to which the first byte is
 * written \param  numBytes                  [IN] number of successive bytes to be written to the
 * registers \param  data                      [IN] bytes to be written \return XLstatus general
 * status information
 */
 ...
 *  ResponseEvent:                         XL_MOST_REGISTER_BYTES
 *  \param  adr                       [IN] address (MAP) of register from which the first byte is
 * read \param  numBytes                  [IN] number of successive bytes to be read \return
 * XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostReadRegister, XLFP_MOSTREADREGISTER,
                (DEFPARAMS, unsigned short adr, unsigned char numBytes));
 ...
 *                                         1 means this bit will be written, 0 means that the bit is
 * not influenced \param  value                     [IN] the byte to be written respecting the
 * parameter mask \return XLstatus                       general status information
 */
 ...
 *  \param  pCtrlMsg                  [IN] structure with all relevant data needed for a transmit
 * request \return XLstatus                       general status information
 */
 ...
 *  The members arbitration, targetAdr, asyncData[1014], length, TODO: prio of pAsyncMsg will be
 * used, all other members don't care for the transmit request.
 *  TODO: arbitration has to be calculated by the sender or will be calculated by the
 * driver/firmware? A XL_MOST_ASYNC_MSG event will be delivered with dir==Tx and txStatus set to
 *  report success or failure of the transmission.\n
 *  ResponseEvent:                         XL_MOST_ASYNC_MSG
 *  \param  pAsyncMsg                 [IN] structure with all relevant data needed for a transmit
 * request \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostAsyncTransmit, XLFP_MOSTASYNCTRANSMIT,
                (DEFPARAMS, XLmostAsyncMsg *pAsyncMsg));
 ...
 *  to either stream audio data from the line in of VN2600 to certain MOST channels allocated
 * before, or to stream audio data from certain MOST channels to the headphone output of VN2600. \n
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostCtrlSyncAudio, XLFP_MOSTCTRLSYNCAUDIO,
                (DEFPARAMS, unsigned int channel[4], unsigned int device, unsigned int mode));
 ...
 *  \return XLstatus general status information
 */
DECL_STDXL_FUNC(xlMostSyncVolume, XLFP_MOSTSYNCVOLUME,
                (DEFPARAMS, unsigned int device, unsigned char volume));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostSyncMute, XLFP_MOSTSYNCMUTE,
                (DEFPARAMS, unsigned int device, unsigned char mute));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostSyncGetVolumeStatus, XLFP_MOSTSYNCGETVOLUMESTATUS,
                (DEFPARAMS, unsigned int device));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostSyncGetMuteStatus, XLFP_MOSTSYNCGETMUTESTATUS,
                (DEFPARAMS, unsigned int device));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostSetLightPower, XLFP_MOSTSETLIGHTPOWER,
                (DEFPARAMS, unsigned char attenuation));
 ...
 *  \param  repeat                    [IN] repetition of light on light off sequence, or repeat==0:
 * stop the generation \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostGenerateLightError, XLFP_MOSTGENERATELIGHTERROR,
                (DEFPARAMS, unsigned long lightOffTime, unsigned long lightOnTime,
...
DECL_STDXL_FUNC(xlMostGenerateLockError, XLFP_MOSTGENERATELOCKERROR,
                (DEFPARAMS, unsigned long unmodTime, unsigned long modTime, unsigned short repeat));
 ...
 *  ResponseEvent:                         none
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param userHandle                 [IN] used to match the response of the driver to
 * the requests of the application if an event is received spontaneously \param
 * pCtrlBusloadConfiguration  [IN] structure containing the ctrl msg used for busload generation and
 * configuration, it's storage has has to be supplied by the caller \return XLstatus general status
 * information
 */
 ...
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param userHandle                 [IN] used to match the response of the driver to
 * the requests of the application if an event is received spontaneously \param numberCtrlFrames
 * [IN] number of busload ctrl messages (0xFFFFFFFF indicates infinite number of messages) \return
 * XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostCtrlGenerateBusload, XLFP_MOSTCTRLGENERATEBUSLOAD,
                (DEFPARAMS, unsigned long numberCtrlFrames));
 ...
 * device's state \param userHandle                 [IN] used to match the response of the driver to
 * the requests of the application if an event is received spontaneously \param
 * pAsyncBusloadConfiguration [IN] structure containing the async msg used for busload generation
 * and configuration, it's storage has has to be supplied by the caller \return XLstatus general
 * status information
 */
 ...
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param userHandle                 [IN] used to match the response of the driver to
 * the requests of the application if an event is received spontaneously \param numberAsyncFrames
 * [IN] number of busload async messages (0xFFFFFFFF indicates infinite number of messages) \return
 * XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostAsyncGenerateBusload, XLFP_MOSTASYNCGENERATEBUSLOAD,
                (DEFPARAMS, unsigned long numberAsyncFrames));

/** \brief Opens a stream (Rx / Tx) for routing synchronous data to or from the MOST bus
 * (synchronous channel). Attention: Has to be called before "xlMostStreamBufferAllocate".
 *  ResponseEvent:                         none
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param userHandle                 [IN] used to match the response of the driver to
 * the requests of the application if an event is received spontaneously \param pStreamOpen [IN]
 * structure containing the stream parameters - it's storage has has to be supplied by the caller
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostStreamOpen, XLFP_MOSTSTREAMOPEN, (DEFPARAMS, XLmostStreamOpen *pStreamOpen));

/** \brief Closes an opened a stream (Rx / Tx) used for routing synchronous data to or from the MOST
 * bus (synchronous channel). ResponseEvent:                         none \param portHandle [IN]
 * handle to port from which the information is requested \param accessMask                 [IN]
 * mask specifying the port's channel from where to get the device's state \param userHandle [IN]
 * used to match the response of the driver to the requests of the application if an event is
 * received spontaneously \param streamHandle               [IN] stream handle returned by
 * xlMostStreamOpen \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostStreamClose, XLFP_MOSTSTREAMCLOSE, (DEFPARAMS, unsigned int streamHandle));

/** \brief Starts the streaming (Rx / Tx) of synchronous data to or from the MOST bus (synchronous
 * channel). Attention: Has to be called after "xlMostStreamOpen and xlMostStreamBufferAllocate"
 * were called. ResponseEvent:                         none \param portHandle                 [IN]
 * handle to port from which the information is requested \param accessMask                 [IN]
 * mask specifying the port's channel from where to get the device's state \param userHandle [IN]
 * used to match the response of the driver to the requests of the application if an event is
 * received spontaneously \param streamHandle               [IN] stream handle returned by
 * xlMostStreamOpen \param syncChannels               [IN] synchronous channels (bytes) used for
 * streaming. \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostStreamStart, XLFP_MOSTSTREAMSTART,
                (DEFPARAMS, unsigned int streamHandle, unsigned char syncChannels[60]));

/** \brief Stops the streaming (Rx / Tx) of synchronous data to or from the MOST bus (synchronous
 * channel). Attention: Has to be called before "xlMostStreamBufferDeallocate". ResponseEvent: none
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param userHandle                 [IN] used to match the response of the driver to
 * the requests of the application if an event is received spontaneously \param streamHandle [IN]
 * stream handle returned by xlMostStreamOpen \return XLstatus                       general status
 * information
 */
DECL_STDXL_FUNC(xlMostStreamStop, XLFP_MOSTSTREAMSTOP, (DEFPARAMS, unsigned int streamHandle));

/** \brief Allocates a buffer for streaming (RX / Tx) of synchronous data to or from the MOST bus
 * (synchronous channel). Attention: Has to be called before "xlMostStreamStart". ResponseEvent:
 * none \param portHandle                 [IN] handle to port from which the information is
 * requested \param accessMask                 [IN] mask specifying the port's channel from where to
 * get the device's state \param userHandle                 [IN] used to match the response of the
 * driver to the requests of the application if an event is received spontaneously \param
 * streamHandle               [IN] stream handle returned by xlMostStreamOpen \param ppBuffer [OUT]
 * pointer to the buffer used for streaming memory allocation is done by the driver has to be
 * released by calling xlMostStreamBufferDeallocate \param pBufferSize               [OUT] buffer
 * size. \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostStreamBufferAllocate, XLFP_MOSTSTREAMBUFFERALLOCATE,
                (DEFPARAMS, unsigned int streamHandle, unsigned char **ppBuffer,
 ...
 * information
 */
DECL_STDXL_FUNC(xlMostStreamBufferDeallocateAll, XLFP_MOSTSTREAMBUFFERDEALLOCATEALL,
                (DEFPARAMS, unsigned int streamHandle));

/** \brief Notifies the driver the next buffer to be used for streaming synchronous data to or from
 * the MOST bus (synchronous channel). ResponseEvent:                         none \param portHandle
 * [IN] handle to port from which the information is requested \param accessMask [IN] mask
 * specifying the port's channel from where to get the device's state \param userHandle [IN] used to
 * match the response of the driver to the requests of the application if an event is received
 * spontaneously \param streamHandle               [IN] stream handle returned by xlMostStreamOpen
...
DECL_STDXL_FUNC(xlMostStreamBufferSetNext, XLFP_MOSTSTREAMBUFFERSETNEXT,
                (DEFPARAMS, unsigned int streamHandle, unsigned char *pBuffer,
 ...
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param userHandle                 [IN] used to match the response of the driver to
 * the requests of the application if an event is received spontaneously \param pStreamInfo [OUT]
 * Pointer to the stream information. \return XLstatus                       general status
 * information
 */
DECL_STDXL_FUNC(xlMostStreamGetInfo, XLFP_MOSTSTREAMGETINFO,
                (DEFPARAMS, XLmostStreamInfo *pStreamInfo));

/** \brief Clears the content of the buffer(s) which are not already sent.
 *  This method is used to clear the content of any TX streaming buffer which has not been sent
 * yet.\n ResponseEvent:                         None \param portHandle                 [IN] handle
 * to port from which the information is requested \param accessMask                 [IN] mask
 * specifying the port's channel from where to get the device's state \param userHandle [IN] used to
 * match the response of the driver to the requests of the application if an event is received
 * spontaneously \param streamHandle               [IN] stream handle returned by xlMostStreamOpen.
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostStreamBufferClearAll, XLFP_MOSTSTREAMBUFFERCLEARALL,
                (DEFPARAMS, unsigned int streamHandle));
...
DECL_STDXL_FUNC(xlFrSetConfiguration, XLFP_FRSETCONFIGURATION,
                (DEFFRPARAM, XLfrClusterConfig *pxlClusterConfig));

/** \brief Get configuration of a FlexRay channel
 *  ResponseEvent:
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * information \param XLfrChannelConfig         [OUT] pointer to the FlexRay channel configuration
 * structure \return XLstatus                       general status information
 */

DECL_STDXL_FUNC(xlFrGetChannelConfiguration, XLFP_FRGETCHANNELCONFIGURATION,
                (DEFFRPARAM, XLfrChannelConfig *pxlFrChannelConfig));

/** \brief Setup the FlexRay mode
 *  ResponseEvent:
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param pxlFrMode                  [IN] structure to the FlexRay mode structure
 * (e.g.: normal-, monitor-, clusterScan mode). \return XLstatus                       general
 * status information
 */
 ...
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param pEventBuffer               [IN] pointer to the startup and sync frame
 *  \return XLstatus                       general status information
 */

DECL_STDXL_FUNC(xlFrInitStartupAndSync, XLFP_FRINITSTARTUPANDSYNC,
                (DEFFRPARAM, XLfrEvent *pEventBuffer));

/** \brief setup the symbol window.
 *  ResponseEvent:
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param frChannel                  [IN] FlexRay channel, like A,B, both... \param
 * symbolWindowMask           [IN] symbol window mask like MTS. \return XLstatus general status
 * information
 */

DECL_STDXL_FUNC(xlFrSetupSymbolWindow, XLFP_FRSETUPSYMBOLWINDOW,
                (DEFFRPARAM, unsigned int frChannel, unsigned int symbolWindowMask));
 ...
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param pEventBuffer               [IN] pointer to the FlexRay TX event \return
 * XLstatus                       general status information
 */
 ...
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param frChannel                  [IN] FlexRay channel. e.g. CHANNEL_A... \param
 * mode                       [IN] transceiver mode. e.g. sleep \return XLstatus general status
 * information
 */

DECL_STDXL_FUNC(xlFrSetTransceiverMode, XLFP_FRSETTRANSCEIVERMODE,
                (DEFFRPARAM, unsigned int frChannel, unsigned int mode));

/** \brief
 *  ResponseEvent:
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param symbolWindow               [IN] defines the symbol window (e.g. MTS).
 *  \return XLstatus                       general status information
 */

DECL_STDXL_FUNC(xlFrSendSymbolWindow, XLFP_FRSENDSYMBOLWINDOW,
                (DEFFRPARAM, unsigned int symbolWindow));

/** \brief
 *  ResponseEvent:
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param mode                       [IN] specifies the spy mode: XL_FR_SPY_MODE_***
 *  \return XLstatus                       general status information
 */
...
DECL_STDXL_FUNC(xlFrSetAcceptanceFilter, XLFP_FRSETACCEPTANCEFILTER,
                (DEFFRPARAM, XLfrAcceptanceFilter *pAcceptanceFilter));

/** \brief The application gets the information, which remote channels are available in the system.
 * The user must provide the memory (pointer to XLdriverConfig structure). \param pDriverConfig
 * [OUT] The remote driver configuration structure. \return XLstatus                        general
 * status information
 */
DECL_STDXL_FUNC(xlGetRemoteDriverConfig, XLGETREMOTEDRIVERCONFIG, (XLdriverConfig * pDriverConfig));

/** \brief The application gets the available RDNI network devices. The buffer is allocated by DLL
 * and can be freed by a call of xlReleaseRemoteDeviceInfo. \param deviceList                 [OUT]
 * Pointer receiving the address of the buffer containing the device configuration information.
 *  \param nbrOfRemoteDevices         [OUT] The number of available network devices.
 *  \param netSearch                  [IN] One of the defines: XL_REMOTE_NET_SEARCH or
 * XL_REMOTE_NO_NET_SEARCH
 *
 *  \return XLstatus                        general status information
 */
DECL_STDXL_FUNC(xlGetRemoteDeviceInfo, XLGETREMOTEDEVICEINFO,
                (XLremoteDeviceInfo * *deviceList, unsigned int *nbrOfRemoteDevices,
                 unsigned int netSearch));
 ...
 *  \return XLstatus                        general status information
 */
DECL_STDXL_FUNC(xlReleaseRemoteDeviceInfo, XLRELEASEREMOTEDEVICEINFO,
                (XLremoteDeviceInfo * *deviceList));

/** \brief The application establishes a connection to a RDNI network device.
 *  \param remoteHandle               [IN] DLL internal handle of the device retrieved by a call to
 * xlGetRemoteDeviceInfo. \param deviceMask                 [IN] not used \param flags [IN] specify
 * if the connection shall be permanent or temporary (until next reboot) One of the defines:
 * XL_REMOTE_ADD_PERMANENT, XL_REMOTE_ADD_TEMPORARY
 *
 *  \return XLstatus                        general status information
 */
DECL_STDXL_FUNC(xlAddRemoteDevice, XLADDREMOTEDEVICE,
 ...
 *  \param remoteHandle               [IN] DLL internal handle of the device retrieved by a call to
 * xlGetRemoteDeviceInfo. \param deviceMask                 [IN] not used \param flags [IN] Server
 * entry removal flag.
 *
 *  \return XLstatus                        general status information
 */
DECL_STDXL_FUNC(xlRemoveRemoteDevice, XLREMOVEREMOTEDEVICE,
                (XLremoteHandle remoteHandle, XLdeviceAccess deviceMask, unsigned int flags));
 ...
 *  \return XLstatus                        general status information
 */
DECL_STDXL_FUNC(xlUpdateRemoteDeviceInfo, XLUPDATEREMOTEDEVICEINFO,
                (XLremoteDeviceInfo * deviceList, unsigned int nbrOfRemoteDevices));

/** \brief Retrieves the hardware type and hardware index of the channels in driver config structure
 that belong to the remote device.
 ...
 *  \return XLstatus                        general status information
 */
DECL_STDXL_FUNC(xlGetRemoteHwInfo, XLGETREMOTEHWINFO,
 ...
 *                                          XL_REMOTE_REGISTER_NONE, XL_REMOTE_REGISTER_CONNECT,
 * XL_REMOTE_REGISTER_TEMP_CONNECT
 *
 *  \return XLstatus                        general status information
 */
DECL_STDXL_FUNC(xlRegisterRemoteDevice, XLREGISTERREMOTEDEVICE,
...
DECL_STDXL_FUNC(xlIoSetTriggerMode, XLIOSETTRIGGERMODE,
                (DEFFRPARAM, XLdaioTriggerMode *pxlDaioTriggerMode));
...
DECL_STDXL_FUNC(xlIoSetDigitalOutput, XLIOSETDIGITALOUTPUT,
                (DEFFRPARAM, XLdaioDigitalParams *pxlDaioDigitalParams));

/** \brief Setup the DAIO ports.
 *  \param pxlDaioSetPort          [IN] Pointer to the XLdaioSetPort structure.
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlIoConfigurePorts, XLIOCONFIGUREPORTS,
                (DEFFRPARAM, XLdaioSetPort *pxlDaioSetPort));

/** \brief Defines the voltage level where a digital signal is measured as logical high and where it
 * is measured as logical low. \param level                   [IN] 10bit value which defines voltage
 * level [mV] for input threshold of digital ports. \return XLstatus                       general
 * status information
 */
...
DECL_STDXL_FUNC(xlIoSetAnalogOutput, XLIOSETANALOGOUTPUT,
                (DEFFRPARAM, XLdaioAnalogParams *pxlDaioAnalogParams));

/** \brief Start measurements.
 *  \param portTypeMask            [IN] Port types on which to start the measurements. Use defines
 * XL_DAIO_PORT_TYPE_MASK_*. \return XLstatus                       general status information
 */
 ...
 * available). \n It is a synchronous mode and either delivers event data immediately, or indicates
 * an error condition with its return value. \param  XLportHandle portHandle:  [IN] was previously
 * fetched by xlOpenPort API \param  XLaccess accessMask:      [IN] determines on which channels an
 * API should work \param  pEventBuffer              [IN] This parameter must point to a buffer to
 * which the driver can copy the next event of the receive queue \return XLstatus general status
 * information
 */
DECL_STDXL_FUNC(xlMost150Receive, XLFP_MOST150RECEIVE,
 ...
 *  \param sourceMask                 [IN] each bit stands for an event source and can separately be
 * set. Use the definitions of the source mask (see XL_MOST150_SOURCE_...). \return XLstatus general
 * status information
 */
DECL_STDXL_FUNC(xlMost150SwitchEventSources, XLFP_MOST150SWITCHEVENTSOURCES,
                (DEFPARAMS, unsigned int sourceMask));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SetDeviceMode, XLFP_MOST150SETDEVICEMODE,
                (DEFPARAMS, unsigned int deviceMode));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SetSPDIFMode, XLFP_MOST150SETSPDIFMODE,
                (DEFPARAMS, unsigned int spdifMode));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SetSpecialNodeInfo, XLFP_MOST150SETSPECIALNODEINFO,
                (DEFPARAMS, XLmost150SetSpecialNodeInfo *pSpecialNodeInfo));

/** \brief Requests one or more parameters of the special node info at once.
 *  ResponseEvent:                         XL_MOST150_SPECIAL_NODE_INFO
 *  \param  requestMask               [IN] contains a mask of parameter to get (see
 * XL_MOST150_SPECIAL_NODE_MASK_CHANGED) \return XLstatus                       general status
 * information
 */
DECL_STDXL_FUNC(xlMost150GetSpecialNodeInfo, XLFP_MOST150GETSPECIALNODEINFO,
                (DEFPARAMS, unsigned int requestMask));

/** \brief Set the frequency of the MOST150 ring.
 *  ResponseEvent:                         XL_MOST150_FREQUENCY
 *  \param  frequency                 [IN] contains the frequency to be set. Only as timing master!
 * (see XL_MOST150_FREQUENCY_...) \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SetFrequency, XLFP_MOST150SETFREQUENCY,
                (DEFPARAMS, unsigned int frequency));
 ...
 *  \param  pCtrlTxMsg                [IN] pointer to structure that contains the control message to
 * be sent \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150CtrlTransmit, XLFP_MOST150CTRLTRANSMIT,
                (DEFPARAMS, XLmost150CtrlTxMsg *pCtrlTxMsg));

/** \brief Transmit a data packet (MDP) on the MOST150 ring.
 *  ResponseEvent:                         XL_MOST150_ASYNC_TX
 *  \param  pAsyncTxMsg               [IN] pointer to structure that contains the MOST Data Packet
 * (MDP) to be sent \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150AsyncTransmit, XLFP_MOST150ASYNCTRANSMIT,
                (DEFPARAMS, XLmost150AsyncTxMsg *pAsyncTxMsg));

/** \brief Transmit a Ethernet packet (MEP) on the MOST150 ring.
 *  ResponseEvent:                         XL_MOST150_ETHERNET_TX
 *  \param  pEthernetTxMsg            [IN] pointer to structure that contains the MOST Ethernet
 * Packet (MEP) to be sent \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150EthernetTransmit, XLFP_MOST150ETHTRANSMIT,
                (DEFPARAMS, XLmost150EthernetTxMsg *pEthernetTxMsg));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150CtrlSyncAudio, XLFP_MOST150CTRLSYNCAUDIO,
                (DEFPARAMS, XLmost150SyncAudioParameter *pSyncAudioParameter));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SyncSetVolume, XLFP_MOST150SYNCSETVOLUME,
                (DEFPARAMS, unsigned int device, unsigned int volume));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SyncGetVolume, XLFP_MOST150SYNCGETVOLUME,
                (DEFPARAMS, unsigned int device));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SyncSetMute, XLFP_MOST150SYNCSETMUTE,
                (DEFPARAMS, unsigned int device, unsigned int mute));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150GetRxLightLockStatus, XLFP_MOST150GETLIGHTLOCKSTATUS,
                (DEFPARAMS, unsigned int fromSpy));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SetTxLightPower, XLFP_MOST150SETTXLIGHTPOWER,
                (DEFPARAMS, unsigned int attenuation));
...
DECL_STDXL_FUNC(xlMost150GenerateLightError, XLFP_MOST150GENLIGHTERROR,
                (DEFPARAMS, unsigned int lightOffTime, unsigned int lightOnTime,
                 unsigned int repeat));
 ...
 *  \param  bufferType                [IN] Bit mask for receive buffer type (control messages and/or
 * packets (MDP/MEP)). \param  bufferMode                [IN] Block or un-block receive buffer
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150ConfigureRxBuffer, XLFP_MOST150CONFIGURERXBUFFER,
                (DEFPARAMS, unsigned int bufferType, unsigned int bufferMode));

/** \brief Defines the control message which should be transmitted with
 * xlMost150CtrlGenerateBusload(). ResponseEvent: \param  pCtrlBusLoad              [IN] pointer to
 * structure that contains the control message \return XLstatus                       general status
 * information
 */
DECL_STDXL_FUNC(xlMost150CtrlConfigureBusload, XLFP_MOST150CTRLCONFIGLOAD,
                (DEFPARAMS, XLmost150CtrlBusloadConfig *pCtrlBusLoad));

/** \brief Starts or stops the control message busload by sending the control message defined with
 * xlMost150CtrlConfigureBusload(). ResponseEvent:                        XL_MOST150_CTRL_BUSLOAD
 ...
 *  \return XLstatus                      general status information
 */
DECL_STDXL_FUNC(xlMost150CtrlGenerateBusload, XLFP_MOST150CTRLGENLOAD,
                (DEFPARAMS, unsigned long numberCtrlFrames));

/** \brief Define the data or Ethernet packet that should be transmitted with
 * xlMost150AsyncGenerateBusload(). ResponseEvent: \param  pAsyncBusLoad             [IN] pointer to
 * structure that contains either the data or the Ethernet packet \return XLstatus general status
 * information
 */
DECL_STDXL_FUNC(xlMost150AsyncConfigureBusload, XLFP_MOST150ASYNCCONFIGLOAD,
                (DEFPARAMS, XLmost150AsyncBusloadConfig *pAsyncBusLoad));

/** \brief Starts or stops the packet busload by sending either the data or Ethernet packet defined
 * with xlMost150AsyncConfigureBusload(). ResponseEvent: XL_MOST150_ASYNC_BUSLOAD \param
 * numberAsyncPackets        [IN] number of data or Ethernet packets to be sent 0:            stop
 * sending < 0xFFFFFFFF: number of packets to be sent 0xFFFFFFFF:   send continuously \return
 * XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150AsyncGenerateBusload, XLFP_MOST150ASYNCGENLOAD,
                (DEFPARAMS, unsigned long numberAsyncPackets));

/** \brief Set the ECL state.
 *  ResponseEvent:                         XL_MOST150_ECL_LINE_CHANGED
 *  \param  eclLineState              [IN] ECL state to be set: XL_MOST150_ECL_LINE_LOW,
 * XL_MOST150_ECL_LINE_HIGH \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SetECLLine, XLFP_MOST150SETECLLINE,
 ...
 * XL_MOST150_ECL_LINE_PULL_UP_ACTIVE \return XLstatus                       general status
 * information
 */
DECL_STDXL_FUNC(xlMost150SetECLTermination, XLFP_MOST150SETECLTERMINATION,
                (DEFPARAMS, unsigned int eclLineTermination));

/** \brief Requests the current ECL state and settings.
 *  This method is asynchronous and requests the event used to get the ECL line state, termination
 * and glitch filter setting. ResponseEvent:                         XL_MOST150_ECL_LINE_CHANGED,
 * XL_MOST150_ECL_TERMINATION_CHANGED, XL_MOST150_ECL_GLITCH_FILTER \return XLstatus general status
 * information
 */
DECL_STDXL_FUNC(xlMost150GetECLInfo, XLFP_MOST150GETECLINFO, (DEFPARAMS));

/** \brief Opens a stream (Rx / Tx) for routing synchronous data to or from the MOST bus
 * (synchronous channel). ResponseEvent:                         XL_MOST150_STREAM_STATE \param
 * portHandle                 [IN] handle to port from which the information is requested \param
 * accessMask                 [IN] mask specifying the port's channel from where to get the device's
 * state \param userHandle                 [IN] used to match the response of the driver to the
 * requests of the application if an event is received spontaneously \param pStreamOpen [IN]
 * structure containing the stream parameters - it's storage has has to be supplied by the caller
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150StreamOpen, XLFP_MOST150STREAMOPEN,
                (DEFPARAMS, XLmost150StreamOpen *pStreamOpen));

/** \brief Closes an opened a stream (Rx / Tx) used for routing synchronous data to or from the MOST
 * bus (synchronous channel). ResponseEvent:                         XL_MOST150_STREAM_STATE \param
 * portHandle                 [IN] handle to port from which the information is requested \param
 * accessMask                 [IN] mask specifying the port's channel from where to get the device's
 * state \param userHandle                 [IN] used to match the response of the driver to the
 * requests of the application if an event is received spontaneously \param streamHandle [IN] stream
 * handle returned by xlMostStreamOpen \return XLstatus                       general status
 * information
 */
DECL_STDXL_FUNC(xlMost150StreamClose, XLFP_MOST150STREAMCLOSE,
                (DEFPARAMS, unsigned int streamHandle));

/** \brief Starts the streaming (Rx / Tx) of synchronous data to or from the MOST bus (synchronous
 * channel). Attention: Has to be called after XL_MOST150_STREAM_STATE "Opened" was received.
 *  ResponseEvent:                         XL_MOST150_STREAM_STATE
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param userHandle                 [IN] used to match the response of the driver to
 * the requests of the application if an event is received spontaneously \param streamHandle [IN]
 * stream handle returned by xlMostStreamOpen \param numConnLabels              [IN] Number of
 * connection labels to stream (only used for Rx streaming, max. 8 labels can be streamed!) \param
 * pConnLabels                [IN] connection label(s) (only used for Rx streaming) \return XLstatus
 * general status information
 */
DECL_STDXL_FUNC(xlMost150StreamStart, XLFP_MOST150STREAMSTART,
                (DEFPARAMS, unsigned int streamHandle, unsigned int numConnLabels,
                 unsigned int *pConnLabels));

/** \brief Stops the streaming (Rx / Tx) of synchronous data to or from the MOST bus (synchronous
 * channel). Attention: Has to be called after XL_MOST150_STREAM_STATE "Started" was received.
 *  ResponseEvent:                         XL_MOST150_STREAM_STATE
 *  \param portHandle                 [IN] handle to port from which the information is requested
 *  \param accessMask                 [IN] mask specifying the port's channel from where to get the
 * device's state \param userHandle                 [IN] used to match the response of the driver to
 ...
 * information
 */
DECL_STDXL_FUNC(xlMost150StreamStop, XLFP_MOST150STREAMSTOP,
                (DEFPARAMS, unsigned int streamHandle));

/** \brief Provides further streaming data to be sent to the MOST bus (synchronous channel).
 *  ResponseEvent:                         XL_MOST150_STREAM_TX_BUFFER as soon as further data is
 * required. \param portHandle                 [IN] handle to port from which the information is
 * requested \param accessMask                 [IN] mask specifying the port's channel from where to
 * get the device's state \param userHandle                 [IN] used to match the response of the
 * driver to the requests of the application if an event is received spontaneously \param
 * streamHandle               [IN] stream handle returned by xlMostStreamOpen \param pBuffer [IN]
 * pointer to the buffer used for streaming \param pNumberOfBytes       [IN]/[OUT] number of bytes
 * contained in the buffer to be used for streaming. In case of not all bytes could be stored, this
 * parameters contains the adjusted number of bytes stored and the function returns an error
 * (XL_ERR_QUEUE_IS_FULL). \return XLstatus                       general status information
 */
 ...
 *  This method is used to clear the content of the driver's TX streaming FIFO which has not been
 * sent yet.\n ResponseEvent:                         None \param portHandle                 [IN]
 * handle to port from which the information is requested \param accessMask                 [IN]
 * mask specifying the port's channel from where to get the device's state \param userHandle [IN]
 * used to match the response of the driver to the requests of the application if an event is
 * received spontaneously \param streamHandle               [IN] stream handle returned by
 * xlMostStreamOpen. \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150StreamClearTxFifo, XLFP_MOST150STREAMCLEARTXFIFO,
                (DEFPARAMS, unsigned int streamHandle));
 ...
 * information
 */
DECL_STDXL_FUNC(xlMost150StreamGetInfo, XLFP_MOST150STREAMGETINFO,
                (DEFPARAMS, XLmost150StreamInfo *pStreamInfo));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150StreamInitRxFifo, XLFP_MOST150STREAMINITRXFIFO,
                (XLportHandle portHandle, XLaccess accessMask));
...
DECL_STDXL_FUNC(xlMost150StreamReceiveData, XLFP_MOST150STREAMRECEIVEDATA,
                (XLportHandle portHandle, XLaccess accessMask, unsigned char *pBuffer,
                 unsigned int *pBufferSize));

/** \brief Controls the bypass stress generation.
 *  \param  XLportHandle portHandle:  [IN] was previously fetched by xlOpenPort API
 *  \param  XLaccess accessMask:      [IN] determines on which channel an API should work
 *  \param  XLuserHandle userHandle:  [IN] used to match the response of the driver to the requests
 * of the application if an event is received spontaneously \param  bypassCloseTime           [IN]
 * duration of bypass close time in [ms] (minimum value: 10 ms) \param  bypassOpenTime [IN] duration
 * of bypass open time in [ms] (minimum value: 10 ms) \param  repeat                    [IN] number
 * of error intervals \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150GenerateBypassStress, XLFP_MOST150GENERATEBYPASSSTRESS,
                (DEFPARAMS, unsigned int bypassCloseTime, unsigned int bypassOpenTime,
                 unsigned int repeat));
 ...
 *  \param  XLaccess accessMask:      [IN] determines on which channel an API should work
 *  \param  XLuserHandle userHandle:  [IN] used to match the response of the driver to the requests
 * of the application if an event is received spontaneously \param  numStates                 [IN]
 * Number of states during the sequence (max. XL_MOST150_ECL_SEQ_NUM_STATES_MAX) \param  pEclStates
 * [IN] Pointer to a buffer containing the ECL sequence states (1: High, 0: Low) \param
 * pEclStatesDuration        [IN] Pointer to a buffer containing the ECL sequence states duration in
 * multiple of 100 祍 (max. value XL_MOST150_ECL_SEQ_DURATION_MAX) NOTE: Both buffers have to have
 * at least the size <numStates> DWORDS! \return XLstatus                       general status
 * information
 */
DECL_STDXL_FUNC(xlMost150EclConfigureSeq, XLFP_MOST150ECLCONFIGURESEQ,
                (DEFPARAMS, unsigned int numStates, unsigned int *pEclStates,
 ...
 *  \param  XLaccess accessMask:      [IN] determines on which channel an API should work
 *  \param  XLuserHandle userHandle:  [IN] used to match the response of the driver to the requests
 * of the application if an event is received spontaneously \param  start                     [IN]
 * Starts (1) or stops (0) the configured ECL sequence \return XLstatus general status information
 */
DECL_STDXL_FUNC(xlMost150EclGenerateSeq, XLFP_MOST150ECLGENERATESEQ,
                (DEFPARAMS, unsigned int start));

/** \brief Configures the glitch filter for detecting ECL state changes.
 *  \param  XLportHandle portHandle:  [IN] was previously fetched by xlOpenPort API
 *  \param  XLaccess accessMask:      [IN] determines on which channel an API should work
 *  \param  XLuserHandle userHandle:  [IN] used to match the response of the driver to the requests
 * of the application if an event is received spontaneously \param  duration                  [IN]
 * Duration (in 祍) of glitches to be filtered. Value range: 50 祍 .. 50 ms (Default: 1 ms) \return
 * XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150SetECLGlitchFilter, XLFP_MOST150SETECLGLITCHFILTER,
                (DEFPARAMS, unsigned int duration));

/** \brief Sets the SSOResult value - needed for resetting the value to 0x00 (No Result) after
 * Shutdown Result analysis has been done. ResponseEvent: XL_MOST150_SSO_RESULT. \param XLportHandle
 * portHandle:  [IN] was previously fetched by xlOpenPort API \param  XLaccess accessMask:      [IN]
 * determines on which channel an API should work \param  XLuserHandle userHandle:  [IN] used to
 * match the response of the driver to the requests of the application if an event is received
 * spontaneously \param  ssoCUStatus               [IN] SSOCUStatus (currently only the value
 * <XL_MOST150_SSO_RESULT_NO_RESULT> is allowed!) \return XLstatus                       general
 * status information
 */
DECL_STDXL_FUNC(xlMost150SetSSOResult, XLFP_MOST150SETSSORESULT,
                (DEFPARAMS, unsigned int ssoCUStatus));
 ...
 *  \param  XLaccess accessMask:      [IN] determines on which channel an API should work
 *  \param  XLuserHandle userHandle:  [IN] used to match the response of the driver to the requests
 * of the application if an event is received spontaneously \return XLstatus general status
 * information
 */
 ...
 * available). \n It is a synchronous mode and either delivers event data immediately, or indicates
 * an error condition with its return value. \param  XLportHandle portHandle:  [IN] was previously
 * fetched by xlOpenPort API \param  ethEventBuffer            [IN] This parameter must point to a
 * buffer to which the driver can copy the next event of the receive queue. The "size" member of
 * this struct specifies the maximum size of the buffer (header plus receive data); upon return, it
 * holds the actual size. \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlEthReceive, XLFP_ETHRECEIVE,
                (XLportHandle portHandle, T_XL_ETH_EVENT *ethEventBuffer));

/** \brief Configures the bypass of two channels.
 *  This method is used to enable the bypass of two channels or to disable the bypass for several
 * channels\n ResponseEvent:                         XL_ETH_CONFIG \param  mode [IN] Bypass state
 * (one of XL_ETH_BYPASS_INACTIVE, XL_ETH_BYPASS_PHY, XL_ETH_BYPASS_MACCORE) \return XLstatus
 * general status information
 */
 ...
 *  This method is asynchronous; a confirmation of the transmit is received via an
 * XL_ETH_TX_OK/XL_ETH_TX_ERROR event. ResponseEvent:                         XL_ETH_TX_OK,
 * XL_ETH_TX_ERROR \param  data:                     [IN] pointer to an Ethernet data frame to be
 * sent \return XLstatus                       general status information
 */
 ...
 *    XLaccess accessMask:                 determines on which channels an API should work.
 *  Common Return Value:
 *    XLstatus:                            common return value of most APIs which indicates whether
 * a command was successfully launched or e.g. whether a command queue overflow happened
 */
 ...
 *  for commands from the driver queue. Each call delivers only one event (if an event is
 * available). \n It is a synchronous mode and either delivers event data immediately, or indicates
 * an error condition with its return value. \param  portHandle:               [IN] was previously
 * fetched by xlOpenPort API \param  pXlA429RxEvt:             [IN] This parameter must point to a
 * buffer to which the driver can copy the next event of the receive queue. The "size" member of
 * this struct specifies the maximum size of the buffer (header plus receive data); upon return, it
 * holds the actual size. \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlA429Receive, XLFP_A429RECEIVE,
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlA429SetChannelParams, XLFP_A429SETCHANNELPARAMS,
                (DEFFRPARAM, XL_A429_PARAMS *pXlA429Params));

/** \brief Transmit a Arinc429 word to the network.
 *  This method is asynchronous; a confirmation of the transmit is received via an
 * XL_A429_EV_TAG_TX_OK/XL_A429_EV_TAG_TX_ERR event. \param  pXlA429MsgTx:             [IN] pointer
 * to an Arinc429 frame to be sent \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlA429Transmit, XLFP_A429TRANSMIT,
                (DEFFRPARAM, unsigned int msgCnt, unsigned int *pMsgCntSent,
 ...
 *  This method returns serial number and license info (license bits) of selected Keyman License
 * dongle \param  boxIndex:                 [IN] index of found Keyman License Dongle (zero based)
...
DECL_STDXL_FUNC(xlGetKeymanInfo, XLFP_GETKEYMANINFO,
                (unsigned int boxIndex, unsigned int *boxMask, unsigned int *boxSerial,
                 XLuint64 *licInfo));
