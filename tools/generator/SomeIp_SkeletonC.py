...
__all__ = ["Gen_SomeIpSkeletonC"]

def Gen_SomeIpSkeletonC(cfg, service, dir, source):
    allStructs = GetStructs(cfg)
    service_name = service["name"]
    source["%sSkeletonC" % (service_name)] = ["%s/%sSkeleton.c" % (dir, service_name)]
    ...
    header_template = env.get_template("someip_skeleton_h.j2")
    header_content = header_template.render(**context)
    
    # Write header file
    header_path = os.path.join(dir, f"{service_name}Skeleton.h")
    with open(header_path, "w") as H:
        H.write(header_content)
    print(f"GEN {os.path.abspath(header_path)}")
    
    # Render source template
    source_template = env.get_template("someip_skeleton_c.j2")
    source_content = source_template.render(**context)
    
    # Write source file
    source_path = os.path.join(dir, f"{service_name}Skeleton.c")
