...
__all__ = ["Gen_VSomeIpSkeleton"]


def Gen_VSomeIpSkeleton(cfg, service, dir, source):
    allStructs = GetStructs(cfg)
    service_name = service["name"]

    source["%sSkeletonV" % (service_name)] = ["%s/%sSkeletonV.cpp" % (dir, service_name)]
    ...
    header_template = env.get_template("vsomeip_skeleton_hpp.j2")
    header_content = header_template.render(**context)

    # Write header file
    header_path = os.path.join(dir, f"{service_name}SkeletonV.hpp")
    with open(header_path, "w") as H:
        H.write(header_content)
    print(f"GEN {os.path.abspath(header_path)}")

    # Render source template
    source_template = env.get_template("vsomeip_skeleton_cpp.j2")
    source_content = source_template.render(**context)

    # Write source file
    source_path = os.path.join(dir, f"{service_name}SkeletonV.cpp")
