...
def Gen_ServerServiceExCpp(service, dir, source):
    mn = toMacro(service["name"])
    C = open("%s/SS_Ex%s.cpp" % (dir, service["name"]), "w")
    source["SS_Ex%sCpp" % (service["name"])] = ["%s/SS_Ex%s.cpp" % (dir, service["name"])]
    GenHeader(C)
    C.write("/* ================================ [ INCLUDES  ] ============================================== */\n")
...
def Gen_ServerService(service, dir, source):
    mn = toMacro(service["name"])
    ...
    for method in service.get("methods", []):
        bName = "%s_%s" % (service["name"], method["name"])
        H.write(
            "Std_ReturnType SomeIp_%s_OnRequest(uint32_t requestId, SomeIp_MessageType* req, SomeIp_MessageType* res);\n"
            % (bName)
        )
    ...
    for method in service.get("methods", []):
        ...
        C.write("    requestId, req->length, req->data[0], req->data[1], req->data[2], req->data[3]));\n")
        C.write("  return E_OK;\n")
        C.write("}\n\n")
        C.write("Std_ReturnType SomeIp_%s_OnFireForgot(uint32_t requestId,SomeIp_MessageType* req) {\n" % (bName))
        C.write(
        ...
        C.write("  return E_OK;\n")
        C.write("}\n\n")
        C.write("Std_ReturnType SomeIp_%s_OnAsyncRequest(uint32_t requestId, SomeIp_MessageType* res) {\n" % (bName))
        C.write("  return E_OK;\n")
        C.write("}\n\n")
        if method.get("tp", False):
    ...
    for egroup in service["event-groups"]:
        bName = "%s_%s" % (service["name"], egroup["name"])
        C.write("void SomeIp_%s_OnSubscribe(boolean isSubscribe, TcpIp_SockAddrType* RemoteAddr) {\n" % (bName))
        C.write(
        ...
        for event in egroup["events"]:
            ...
            C.write("Std_ReturnType %s_notify(uint8_t *data, uint32_t length) {\n" % (beName))
    ...
    source["SS_%sCpp" % (service["name"])] = ["%s/SS_%s.cpp" % (dir, service["name"])]
    GenHeader(C)
    C.write("/* ================================ [ INCLUDES  ] ============================================== */\n")
    C.write('extern "C" {\n')
    C.write('#include "SS_%s.h"\n' % (service["name"]))
    ...
    for method in service.get("methods", []):
        ...
        C.write(
            "  return as::usomeip::server::on_request(SOMEIP_RX_METHOD_%s, requestId, req, res);\n" % (toMacro(bName))
        )
        C.write("}\n\n")
        C.write("Std_ReturnType SomeIp_%s_OnFireForgot(uint32_t requestId,SomeIp_MessageType* req) {\n" % (bName))
        C.write(
            "  return as::usomeip::server::on_fire_forgot(SOMEIP_RX_METHOD_%s, requestId, req);\n" % (toMacro(bName))
        )
        C.write("}\n\n")
        C.write("Std_ReturnType SomeIp_%s_OnAsyncRequest(uint32_t requestId, SomeIp_MessageType* res) {\n" % (bName))
        C.write(
            "  return as::usomeip::server::on_async_request(SOMEIP_RX_METHOD_%s, requestId, res);\n" % (toMacro(bName))
        )
        C.write("}\n\n")
        if method.get("tp", False):
            C.write(
            ...
            C.write(
                "  return as::usomeip::server::on_method_tp_rx_data(SOMEIP_RX_METHOD_%s, requestId, msg);\n"
                % (toMacro(bName))
            )
            C.write("}\n\n")
            C.write(
            ...
            C.write(
                "  return as::usomeip::server::on_method_tp_tx_data(SOMEIP_RX_METHOD_%s, requestId, msg);\n"
                % (toMacro(bName))
            )
            C.write("}\n\n")
    ...
    for egroup in service["event-groups"]:
        ...
        C.write("void SomeIp_%s_OnSubscribe(boolean isSubscribe, TcpIp_SockAddrType* RemoteAddr) {\n" % (bName))
        C.write(
            "return as::usomeip::server::on_subscribe(SD_EVENT_HANDLER_%s, isSubscribe, RemoteAddr);\n"
            % (toMacro(bName))
        )
        C.write("}\n\n")
        for event in egroup["events"]:
            ...
            if event.get("tp", False):
                ...
                C.write("  return as::usomeip::server::on_event_tp_tx_data(requestId, msg);\n")
                C.write("}\n\n")
    C.write("}\n")
    C.close()
...
def Gen_ClientServiceExCpp(service, dir, source):
    mn = toMacro(service["name"])
    C = open("%s/CS_Ex%s.cpp" % (dir, service["name"]), "w")
    source["CS_Ex%sCpp" % (service["name"])] = ["%s/CS_Ex%s.cpp" % (dir, service["name"])]
    GenHeader(C)
    C.write("/* ================================ [ INCLUDES  ] ============================================== */\n")
    C.write('#include "usomeip/usomeip.hpp"\n')
...
def Gen_ClientService(service, dir, source):
    mn = toMacro(service["name"])
    ...
    source["CS_%s" % (service["name"])] = ["%s/CS_%s.c" % (dir, service["name"])]
    GenHeader(C)
    C.write("/* TODO: This is default demo code */\n")
    C.write("/* ================================ [ INCLUDES  ] ============================================== */\n")
    C.write('#include "CS_%s.h"\n' % (service["name"]))
    ...
    for method in service.get("methods", []):
        ...
        C.write("        requestId, res->length, res->data[0], res->data[1], res->data[2], res->data[3]));\n")
        C.write("  return E_OK;\n")
        C.write("}\n\n")
        C.write("Std_ReturnType SomeIp_%s_OnError(uint32_t requestId, Std_ReturnType ercd) {\n" % (bName))
        C.write('  ASLOG(%s, ("%s OnError %%X: %%d\\n", requestId, ercd));\n' % (mn, method["name"]))
        C.write("  return E_OK;\n")
        C.write("}\n\n")
        if method.get("tp", False):
            Gen_MethodRxTxTp(C, service, method)
    for egroup in service["event-groups"]:
        for event in egroup["events"]:
            ...
            C.write("        requestId, evt->length, evt->data[0], evt->data[1], evt->data[2], evt->data[3]));\n")
            C.write("  return E_OK;\n")
            C.write("}\n\n")
            if event.get("tp", False):
    ...
    source["CS_%sCpp" % (service["name"])] = ["%s/CS_%s.cpp" % (dir, service["name"])]
    GenHeader(C)
    C.write("/* TODO: This is default demo code */\n")
    C.write("/* ================================ [ INCLUDES  ] ============================================== */\n")
    C.write('extern "C" {\n')
    ...
    C.write("/* ================================ [ FUNCTIONS ] ============================================== */\n")
    C.write("void SomeIp_%s_OnAvailability(boolean isAvailable) {\n" % (service["name"]))
    C.write("  as::usomeip::client::on_availability(SOMEIP_CSID_%s, isAvailable);\n" % (mn))
    C.write("}\n\n")
    for method in service.get("methods", []):
        bName = "%s_%s" % (service["name"], method["name"])
        C.write("Std_ReturnType SomeIp_%s_OnResponse(uint32_t requestId, SomeIp_MessageType* res) {\n" % (bName))
        C.write("  return as::usomeip::client::on_response(SOMEIP_TX_METHOD_%s, requestId, res);\n" % (toMacro(bName)))
        C.write("}\n\n")
        C.write("Std_ReturnType SomeIp_%s_OnError(uint32_t requestId, Std_ReturnType ercd) {\n" % (bName))
        C.write("  return as::usomeip::client::on_error(SOMEIP_TX_METHOD_%s, requestId, ercd);\n" % (toMacro(bName)))
        C.write("}\n\n")
        if method.get("tp", False):
            C.write(
                "Std_ReturnType SomeIp_%s_OnTpCopyRxData(uint32_t requestId, SomeIp_TpMessageType *msg) {\n" % (bName)
            ...
            C.write(
                "  return as::usomeip::client::on_method_tp_rx_data(SOMEIP_TX_METHOD_%s, requestId, msg);\n"
                % (toMacro(bName))
            )
            C.write("}\n\n")
            C.write(
            ...
            C.write(
                "  return as::usomeip::client::on_method_tp_tx_data(SOMEIP_TX_METHOD_%s, requestId, msg);\n"
                % (toMacro(bName))
            )
            C.write("}\n\n")
    for egroup in service["event-groups"]:
        for event in egroup["events"]:
            ...
            C.write(
                "  return as::usomeip::client::on_notification(SOMEIP_RX_EVT_%s, requestId, evt);\n" % (toMacro(beName))
            )
            C.write("}\n\n")
            if event.get("tp", False):
                C.write(
                ...
                C.write(
                    "  return as::usomeip::client::on_event_tp_rx_data(SOMEIP_RX_EVT_%s, requestId, msg);\n"
                    % (toMacro(beName))
                )
                C.write("}\n\n")
    C.close()
