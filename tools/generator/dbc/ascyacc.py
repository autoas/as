...
def p_sg(p):
    ...
    p[0] = {'name':p[2],'start':p[4],'size':p[6],'endian':'big' if p[8] == 0 else 'little','sign':p[9],'factor':p[11],'offset':p[13],'min':p[16],'max':p[18],'unit':p[20],'node':p[21]}
